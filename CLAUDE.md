...
A third tab between İstanbulite and Hane, and deliberately not a feature: twelve numbered frames
(`assets/project/`, generated by `scripts/make-project-frames.py`) scrubbed by the finger, with no
map, no data, no page swap and none of the shared transition machinery behind them. **The book can
only come to rest on the first frame or the last** — let go anywhere between and it runs to
whichever end is nearer, because there is no stopping in the middle and nothing to press there.
