...
### `project.html` — the Proje tab, which is a rig

A third tab between İstanbulite and Hane, and deliberately not a feature: twenty-four numbered
frames (`assets/project/`, generated by `scripts/make-project-frames.py`) scrubbed by the finger,
with no map, no data, no page swap and none of the shared transition machinery behind them. **The
book can only come to rest on the first frame or the last** — let go anywhere between and it runs
to whichever end is nearer, because there is no stopping in the middle and nothing to press there.

**A throw decides by direction, not by distance** (`FLICK`, the app's own 0.55 px/ms), which is what
keeps a long book crossable at all: past that speed the release commits to whichever way the thumb
was going, and only a slow release falls back to whichever end is nearer. And **the thumb distance
...
It exists to separate two questions that kept getting confused. If the flip is not smooth here, it
is the flip book's fault. If it is smooth here — and it is: 75 frames under the finger at 17ms
each, none dropped, on a 4×-throttled CPU — then whatever made a real journey rough is something
else, and the rig says where to look.

Three things it settles, which the frames carry printed on them: the big number says which frame is
...
`kutuphane-map-mobile.png` and frame 12 is `istanbul-map-mobile.png`, with frames 2–11 the drawn
run between them (frame 7 is still a numbered placeholder) and 13–24 numbered placeholders waiting
to be drawn. One `FIT` constant switches
...
**The cast is the point of the rig now.** The two ends carry real things — buttons, panels, the
page's own furniture — and `CAST` in project.html is where each one is declared: an element, the
slide it belongs to (`live`), and a pose per slide. Two rules make them work:
