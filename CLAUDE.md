...
- `neighborhood` — current residence; must be one of the 39 Istanbul districts (`istanbul_disi` is **not** allowed here — if you don't live in Istanbul you don't get an account).
- `birth_neighborhood` — where the user was born; same 39 districts plus `istanbul_disi` for users born outside Istanbul.
...
- 40 rows: the 39 Istanbul districts plus `istanbul_disi` (İstanbul Dışı) — see "Neighborhood IDs"
  below for the original 25 vs. the 14 added by `db/neighborhoods_v2_missing_districts.sql`.
...
The `neighborhoods` table holds all 39 official Istanbul ilçe (used as `neighborhood` values in
Supabase) plus `istanbul_disi`. The original 25 — the set with a traced hit-region on the
Kahvehane/Anahane SVG maps — are:
...
These IDs match SVG group element data attributes in `index.html` and the values stored in the
Supabase `articles` table.

`db/neighborhoods_v2_missing_districts.sql` adds the 14 that were missing from the table entirely —
a member born in, or living in, one of these had nothing to pick on the signup form:

```
adalar, arnavutkoy, avcilar, beylikduzu, buyukcekmece, catalca, esenyurt,
kartal, pendik, sancaktepe, silivri, sultanbeyli, sile, tuzla
```

**These 14 have no map presence yet** — no traced hit-region on `assets/map/istanbul-map.svg`, no
painted home map in `assets/map/home/`, and they're absent from the hardcoded `NB_NAMES` /
`NEIGHBORHOOD_NAMES` fallback tables duplicated across `profile-card.js`, `kahvehane.html`,
`project.html`, `sozcel.html`, `admin.html` and `bulmaca.html` (those are a display fallback for
when the live `neighborhoods` fetch hasn't landed yet, not what the signup dropdown reads from). A
member whose current residence is one of these 14 can sign up and use the app today, but their own
district won't highlight on the map. Drawing those hit-regions and painted maps, and extending the
fallback tables, is the remaining half of "Add a new neighborhood" below for these 14.
...
**And a fifth is `assets/frame.png`'s own ring** (`ist-frame-ink` in map-ink.js's `LADDERS`, gated
on `html.ist-map-ink` like the rest). The hexagon frame is drawn with a lit face and a shadow face,
two flat tones exactly like every ladder above, bound to two ordinary tokens declared next to
`--ink` — `--frame-hi` / `--frame-lo`, one pair per palette. Mono is left as drawn (the artwork's
own two grays, `#3d3d3d` / `#181818`, day and night alike); earth reads it as an actual
dark-brown/light-brown pair, and needs no colors of its own for it — `--ink` and `--brown-1` already
are that pair. Sözcel's board and keyboard are absent from the ladder entirely — pinned to `#000`,
where "the lit face" and "the shadow face" are both just black, so there is nothing for a bevel to
show and that ring stays the plain flat mask forever (see frames.css's own note).

**The petek's own rings are the one deliberate exception**, carved out in palette.css at higher
specificity: their color IS the meaning (the five-tone distance ladder, and red for whoever is
named on the top bar — see profile-card.css and "The petek" section), so a shared bevel would
flatten that distinction into one look for everybody. They keep the plain flat mask + solid
`--hexframe-stroke` color, untouched by the ladder above.
